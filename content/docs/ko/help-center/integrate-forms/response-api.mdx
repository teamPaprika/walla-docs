---
title: "응답 API"
description: "Walla Open API를 사용하여 폼 데이터와 응답을 프로그래밍 방식으로 조회하고 관리할 수 있습니다."
---

## Walla Open API 가이드

Walla Open API를 사용하여 폼 데이터와 응답을 프로그래밍 방식으로 조회하고 관리할 수 있습니다.
...
향후 변경 사항이 있을 경우 새로운 버전으로 제공될 수 있습니다.
...
Walla API는 API Key 인증 방식을 사용합니다. 모든 API 요청 시 헤더에 API 키를 포함해야 합니다.
...
본인이 권한을 가진 전체 워크스페이스 목록을 조회합니다.
...
본인이 권한을 가진 모든 워크스페이스의 전체 폼 목록을 조회합니다.
...
특정 필드의 상세 정보를 조회합니다. 필드의 모든 속성, 검증 규칙, 출력 스키마 등을 포함합니다.
...
필드 타입별 출력 스키마 매핑 정보를 조회합니다. 응답 데이터를 해석할 때 각 필드 타입이 어떤 데이터 형식을 반환하는지 이해하는 데 도움이 됩니다.
...
특정 폼의 응답 목록을 페이지네이션하여 조회합니다. `customerKey` 파라미터를 사용하여 특정 고객의 응답만 필터링할 수 있습니다.
...
응답 데이터의 각 필드 값은 필드 타입에 따라 다른 형식을 가집니다.
...
`customerKey`는 Form Delivery 기능 사용 시 각 수신자를 식별하기 위해 자동 생성되는 값입니다.

- 발송 상태 추적 (발송됨 → 열람 → 응답완료)에 사용됩니다.
- 응답 데이터 필터링 시 사용할 수 있습니다.
- Form Delivery를 사용하지 않는 경우 비어있을 수 있습니다.
## 에러 응답

API 요청이 실패하면 표준 HTTP 상태 코드와 함께 에러 메시지가 반환됩니다.
...
- 외부 시스템의 고객 ID를 Walla의 `customerKey`로 사용합니다.
- `GET /forms/{formId}/responses?customerKey={customerKey}` API로 해당 고객의 응답 데이터를 조회합니다.
- 응답 목록의 `totalCount`가 0보다 크면 응답이 존재하는 것입니다.
## 2. 폼 응답 목록 조회
