...
`WallaField`는 **모듈 최상위에서** 생성하세요. 호스트는 iframe의 `load` 이벤트에 맞춰 포트를 전달합니다. `postMessage`는 재배달되지 않습니다. `useEffect`나 `onMounted` 같은 마운트 훅 안에서 생성하면 포트를 놓칠 수 있습니다 ([라이프사이클 & 통신 → 핸드셰이크 계약](./custom-fields-lifecycle) 참조).
...
- 콜백 안의 느린 동기 작업이 5초 ack 타임아웃을 넘길 수 있습니다. 그러면 호스트가 필드를 죽은 것으로 판정합니다. 비동기 작업은 문제되지 않습니다.
...
| `init` ack 타임아웃 | 5초 | `onInit`이 이 시간 안에 반환해야 함 |
| 포트 재전송 간격 | 5초 × 4회 | 필드가 응답할 때까지 호스트가 포트 재전송 |
