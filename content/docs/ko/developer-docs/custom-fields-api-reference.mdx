...
`WallaField`는 **모듈 최상위에서** 생성하세요. 호스트는 iframe의 `load` 이벤트에 맞춰 포트를 전달하고 `postMessage`는 재배달되지 않으므로, `useEffect`/`onMounted` 같은 마운트 훅 안에서 생성하면 포트를 놓칠 수 있습니다 ([라이프사이클 & 통신 → 핸드셰이크 계약](./custom-fields-lifecycle) 참조).
...
- 콜백 안의 느린 동기 작업이 5초 ack 타임아웃을 넘기면 호스트가 필드를 죽은 것으로 판정합니다 — 비동기 작업을 시작하는 것은 무방합니다
