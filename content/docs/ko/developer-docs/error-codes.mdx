...
| 401 | Unauthorized | 인증 실패 (API 키 누락 또는 유효하지 않음) |
| 403 | Forbidden | 권한 부족 (해당 리소스에 접근할 수 없음) |
...
API 키가 없거나 유효하지 않아 인증에 실패한 경우 발생합니다. 응답에는 `WWW-Authenticate: ApiKey realm="walla-open-api"` 헤더가 함께 반환됩니다.
...
2. **잘못되었거나 만료된 API 키**
   ...
   ```
   **해결 방법**: API 키가 올바르고 만료·취소되지 않았는지 확인하세요.
...
인증은 통과했지만 해당 리소스에 접근할 권한이 없는 경우 발생합니다.
...
1. **권한 없는 워크스페이스·폼 접근**
   ...
   ```
   **해결 방법**: 요청한 워크스페이스 또는 폼이 본인 팀의 리소스인지 확인하고, 필요하면 워크스페이스 관리자에게 접근 권한을 요청하세요.
