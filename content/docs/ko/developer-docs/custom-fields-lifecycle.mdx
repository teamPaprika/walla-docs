...
연결 과정 자체는 자동이지만, 필드가 지켜야 할 계약이 세 가지 있습니다. 호스트는 `init()` 응답을 필드의 생존 신호로 사용하기 때문에, 이 계약을 어기면 정상 동작하는 필드가 죽은 것으로 판정됩니다.
...
호출하면 런타임 에러가 발생합니다. 아래 예제는 호스트 구현이 완료된 뒤를 위한 참고용입니다.
</Callout>
