...
연결 과정 자체는 자동이지만, 필드는 세 가지 계약을 지켜야 합니다. 호스트는 `init()` 응답을 생존 신호로 사용합니다. 계약 하나를 어기면 정상 동작하는 필드가 죽은 것으로 판정됩니다.
...
### `WallaField`는 모듈 최상위에서 생성하세요

호스트는 iframe의 `load` 이벤트에 맞춰 `MessagePort`를 전달합니다. `postMessage`는 나중에 등록된 리스너에게 **재배달되지 않으므로**, 프레임워크 마운트 훅(React `useEffect`, Vue `onMounted`) 안에서 SDK를 생성하면 포트를 놓칠 수 있습니다.
...
호스트가 5초 간격으로 4회까지 포트를 다시 보내 대개는 연결되지만, 마지막 시도가 약 15초 시점이라 그 뒤에 붙는 리스너는 포트를 받지 못합니다. 그동안 응답자는 스피너만 보게 되고, 20초 ready 예산이 끝나면 실패로 보고됩니다. 재시도는 안전망일 뿐 권장 패턴이 아닙니다.

### `onInit`은 빨리 반환하세요

호스트의 `init()` 호출은 생존 확인을 겸합니다. SDK가 `onInit` 콜백을 **동기로** 호출하고, 그 호출이 반환해야 호스트가 필드를 살아있다고 판정합니다. `onInit` 안의 느린 **동기** 작업은 이 응답을 지연시키며, ack 타임아웃을 넘기면 정상 필드가 죽은 것으로 판정됩니다.
...
비동기 작업을 시작하는 것은 무방합니다 — 콜백 안의 동기 작업만 문제가 됩니다.
...
<Callout type="warning" title="uploadBlob은 현재 호스트에 미구현 상태입니다">
`uploadBlob()`을 호출하면 런타임 에러가 발생합니다. 아래 예제는 호스트가 이 메서드를 구현할 때까지는 참고용입니다.
</Callout>
...
<Callout type="warning" title="recoverable: false는 되돌릴 수 없습니다">
호스트가 필드를 화면에서 제거하고 다시 살아나지 않습니다 — 이후 iframe 안에서 복구하더라도 화면에 표시되지 않습니다. 이 응답자에게 정말로 계속할 수 없을 때만 사용하세요. 재시도 가능한 문제(조회 실패, 일시적 네트워크 오류)는 `recoverable: true`로 보내면 필드 위에 안내만 뜨고 필드는 계속 동작합니다.
</Callout>
